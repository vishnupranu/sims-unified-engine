...
const colleges = [
  { name: "SIMS College of Pharmacy", path: "/colleges/pharmacy" },
  { name: "SIMS College of Nursing", path: "/colleges/nursing" },
  { name: "SIMS College of Physiotherapy", path: "/colleges/physiotherapy" },
  { name: "SIMS College of Education", path: "/colleges/education" },
  { name: "KSIMS College of Physiotherapy", path: "/colleges/ksims-physiotherapy" },
  { name: "SIMS College of Life Sciences", path: "/colleges/life-sciences" },
  { name: "Aswini College of Nursing", path: "/colleges/aswini-nursing" },
];

const navLinks = [
  { name: "Home", path: "/" },
  { name: "About Us", path: "/about" },
  { name: "Colleges", path: "#", hasDropdown: true },
        ...
        <div className="container flex items-center justify-between py-4">
          ...
          <Link to="/" className="flex items-center gap-3">
            ...
            <div className="hidden md:block">
              ...
              <p className="text-xs text-muted-foreground">Excellence in Healthcare Education</p>
            </div>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-1">
            {navLinks.map((link) => (
              <div key={link.name} className="relative group">
                {link.hasDropdown ? (
                  <button
                    className={`flex items-center gap-1 px-4 py-2 rounded-lg font-medium transition-colors hover:bg-muted hover:text-primary ${
                      location.pathname.includes("/colleges") ? "text-primary bg-muted" : "text-foreground"
                    }`}
                  ...
                  <Link
                    to={link.path}
                    className={`px-4 py-2 rounded-lg font-medium transition-colors hover:bg-muted hover:text-primary ${
                ...
                {link.hasDropdown && (
                  <div
                    className={`absolute top-full left-0 mt-1 w-72 bg-card rounded-lg shadow-elegant border border-border overflow-hidden transition-all duration-200 ${
                      collegesOpen ? "opacity-100 visible translate-y-0" : "opacity-0 invisible -translate-y-2"
                    }`}
                    onMouseEnter={() => setCollegesOpen(true)}
                    onMouseLeave={() => setCollegesOpen(false)}
                  >
                    {colleges.map((college) => (
                      <Link
                        key={college.path}
                        to={college.path}
                        className="block px-4 py-3 text-sm hover:bg-muted transition-colors border-b border-border last:border-0"
                      >
                        {college.name}
                      </Link>
                    ))}
                ...
                <div key={link.name}>
                  {link.hasDropdown ? (
                    <div className="space-y-1">
                      <button
                        className="flex items-center justify-between w-full px-4 py-3 rounded-lg font-medium hover:bg-muted"
